...
docker run --rm -it -d --dns 10.0.1.4 --network=none -v "/tmp/quiche-server-data-$1":/server-data --name quiche-client scouture/protobuf-quiche /bin/bash
...
docker run --rm -it -d --dns 10.0.1.4 --network=none -v "/tmp/quiche-server-data-$1":/server-data --name quiche-server scouture/protobuf-quiche /bin/bash
#docker run --shm-size=4g --rm -it -d  --network=none --entrypoint "/bin/bash" -v "/tmp/browsertime-$1":/browsertime --dns 10.0.1.4 --cap-add=SYS_NICE --name $1-browsertime constantin/browsertime
