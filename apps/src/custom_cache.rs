...
fn unchunk(body: &[u8]) -> Vec<u8> {
    ...
    loop {
        ...
        // &tmp[(tmp.len() - 2)..] == "\r\n"
        if tmp.len() >= 2 && tmp[tmp.len() - 2] == '\r' && tmp[tmp.len() - 1] == '\n' {
            let chunk_size: String = tmp[..tmp.len() - 2].to_vec().into_iter().collect();
            ...
            match i64::from_str_radix(&chunk_size, 16) {
                Ok(cl) => {
                    ...
                    let tail = String::from_utf8(body[offset..offset + 2].to_vec()).unwrap();
                    offset += 2;
...
pub fn get_cache() -> HashMap<CacheKey, Vec<CacheEntry>> {
    ...
    'foreach_file: for pb_file in pb_files {
        ...
        let rr: http_record::RequestResponse = protobuf::Message::parse_from_bytes(&buffer).unwrap();
    
        // set https var
        // let https = (rr.scheme == http_record::request_response::Scheme::HTTPS);
        let https = matches!(rr.scheme.unwrap().unwrap(), http_record::request_response::Scheme::HTTPS);
        
    
        // build map of requests String: (String, String)
        let mut request_headers: HashMap<String, (String, String)> = HashMap::new();
        for rheader in &rr.request.header {
            let key = String::from_utf8(rheader.key.as_deref().unwrap().to_vec()).unwrap();
            let key_stripped = key.trim().to_lowercase();
            let val = String::from_utf8(rheader.value.as_deref().unwrap().to_vec()).unwrap();
        ...
        let response_line = String::from_utf8(rr.response.first_line.as_deref().unwrap().to_vec()).unwrap();
        let request_line = String::from_utf8(rr.request.first_line.as_deref().unwrap().to_vec()).unwrap();
        ...
        println!("Cache key: [{}] [{}] [{}] [{}]", req_method, req_keyuri, req_host, https);
        ...
        // insert into cache
        let cache_entries = cache.entry(cache_key).or_insert(Vec::<CacheEntry>::new());
